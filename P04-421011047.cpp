...
int main()
{
	...
	CrearPrismaRectangular(); //Para el cuerpo de la grua 
	CreateShaders();
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// ------------------------------------ Aqui comienza la grua -----------------------------
		
		//AQUÍ SE DIBUJA LA CABINA, LA BASE, LAS 4 LLANTAS
		//Cabina de contorl - Blanca emulando el cristal -- base
		color = glm::vec3(1.0f, 1.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = glm::translate(model, glm::vec3(2.0f, 6.0f, 8.0f));
		...
		// Cuerpo de la grua - Caja trasera roja
		color = glm::vec3(1.0f, 0.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = glm::translate(model, glm::vec3(2.0f, -0.5f, 0.0f));
		model = glm::scale(model, glm::vec3(3.0f, 1.2f, 1.9f));
		...
		// ------------------------------------ Figuras para completar la grua y comenzar el orden jerarquico -----------------------------
		...
		//Llantas subordinadas a la base (cuerpo de la grúa)
		//La orientacion de las llantas la estoy considerando si viera la grua desde arriba con el brazo apuntando hacia arriba

		model = glm::mat4(1.0); // Reiniciar la matriz modelo
		model = glm::translate(model, glm::vec3(2.0f, -0.5f, 0.0f)); // Posición del cuerpo de la cabina
		modelbaseaux = model; // Guardamos esta matriz como matriz base para las llantas

		// Llanta 1 - Trasera Izquierda (articulación 5)
		model = modelbaseaux; // Partimos de la posición de la base
		model = glm::translate(model, glm::vec3(12.5f, 4.5f, 13.3f)); 

		// Colocadas para cambiar los valores de posicion heredados y enderezar las llantas
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));

		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion5()), glm::vec3(0.0f, 1.0f, 0.0f)); 
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f)); // Escalamos para alinear la llanta
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.2f, 0.2f, 0.2f); // Color negro/gris oscuro para las llantas
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[2]->RenderMeshGeometry(); // Cilindro para la llanta

		// Llanta 2 - Delantera Izquierda (articulación 6)
		model = modelbaseaux; // Partimos de la posición de la base
		model = glm::translate(model, glm::vec3(2.0f, 4.5f, 13.3f)); 
		...
		// Llanta 3 - Trasera Derecha (articulación 7)
		model = modelbaseaux; // Partimos de la posición de la base
		model = glm::translate(model, glm::vec3(12.5f, 4.5f, 2.7f)); 

		// Colocadas para cambiar los valores de posicion heredados y enderezar las lantas
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		...
		// Llanta 4 - Delantera Derecha (articulación 8)
		model = modelbaseaux; // Partimos de la posición de la base
		model = glm::translate(model, glm::vec3(2.0f, 4.5f, 2.7f)); 
		...
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[2]->RenderMeshGeometry();
		// ------------------------------------ Aqui termina la grua -----------------------------


		/*
		//--------------------------------------------------- AQUÍ EMPIEZA LA TORTUGA---------------------------------------------------
		...
		model = glm::scale(model, glm::vec3(7.0f, 1.9f, 5.8f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		modelbaseaux = model;
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		sp.render(); // Usamos la esfera para el caparazón 
		//meshList[5]->RenderMesh();

		//PATA 1 - articulación 1
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f));
		...
		modelaux = model;
		model = glm::scale(model, glm::vec3(3.0f, 1.5f, 1.9f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		sp.render(); // Usamos la esfera para el caparazón 
		...
		// PATA 1 - Trasera Izquierda (articulación 5)
		model = modelbaseaux; // Partimos de la posición de la base
		model = glm::translate(model, glm::vec3(20.0f, 0.20f, -15.0f)); // Posicionamos la llanta relativa a la base 


		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion5()), glm::vec3(0.0f, 1.0f, 0.0f)); 
		model = glm::scale(model, glm::vec3(5.0f, 10.0f, 5.0f)); 
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 1.0f, 0.0f); // Color negro/gris oscuro para las llantas
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[0]->RenderMesh(); // Cilindro para la llanta

		// PATA 2 - Delantera Izquierda (articulación 6)
		model = modelbaseaux; // datos dE la base 
		model = glm::translate(model, glm::vec3(0.10f, 0.20f, 15.0f)); 
		...
		// PATA 3 - Trasera Derecha (articulación 7)
		model = modelbaseaux; // datos d ela base 
		model = glm::translate(model, glm::vec3(20.0f,0.20f, 15.0f)); 
		...
		// PATA 4 - Delantera Derecha (articulación 8)
		model = modelbaseaux; // datos d ela base 
		model = glm::translate(model, glm::vec3(0.10f, 0.20f, -15.0f)); 
		...
		// cOLA -  (articulación 9)
		model = modelbaseaux; // datos d ela base 
		model = glm::translate(model, glm::vec3(40.0f, 5.0f, 2.0f)); 
