...
Window::Window()
{
	...
	trofeoLightOn = true; // Luz del trofeo encendida 
	lampLightOn = true; // Inicializa la luz encendida
...
Window::Window(GLint windowWidth, GLint windowHeight)
{
	...
	// Inicializa la variable de control de la luz
	lampLightOn = true;
...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	// Tecla P para encender/apagar la luz de la lámpara
	if (key == GLFW_KEY_P && action == GLFW_PRESS) {
		theWindow->lampLightOn = !theWindow->lampLightOn;
		printf("Luz de la lampara: %s\n", theWindow->lampLightOn ? "prendida" : "apagada");
	}

	// Tecla T para encender/apagar la luz de la lámpara
	if (key == GLFW_KEY_T && action == GLFW_PRESS) {
		theWindow->trofeoLightOn = !theWindow->trofeoLightOn;
		printf("Luz del trofeo: %s\n", theWindow->trofeoLightOn ? "prendida" : "apagada");
	}
	...
	if (key == GLFW_KEY_Z)
	{
		theWindow->isMovingForward = true;
		theWindow->isMovingBackward = false;
	...
	if (key == GLFW_KEY_X)
	{
		theWindow->isMovingForward = false;
		theWindow->isMovingBackward = true;
		// Calcular el movimiento basado en la rotación actual
		theWindow->posX -= sin(glm::radians(theWindow->autoRotY)) * 1.0f;
		theWindow->posZ -= cos(glm::radians(theWindow->autoRotY)) * 1.0f;
	}
	// Si no hay movimiento, apagar ambos focos
	if (action == GLFW_RELEASE && (key == GLFW_KEY_Z || key == GLFW_KEY_X))
	{
		theWindow->isMovingForward = false;
		theWindow->isMovingBackward = false;
	}
